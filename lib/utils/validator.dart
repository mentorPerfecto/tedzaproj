...
class Validators {
  ...
  final  validateBgCheckPattern = RegExp(r'^BC-\d{7}$', caseSensitive: false);
  final  validateTenantOnboardPattern =RegExp(r'^TO-\d{7}$',  caseSensitive: false);
  ...
  String? validateBgCheckCode(String? value) {
    if (value!.trim().isEmpty) {
      return 'Field cannot be empty';
    } else if (!validateBgCheckPattern.hasMatch(value.trim())) {
      return 'Code is invalid';
    }
    return null;
  }

  String? validateTenantOnboardToken(String? value) {
    if (value!.trim().isEmpty) {
      return 'Field cannot be empty';
    } else if (!validateTenantOnboardPattern.hasMatch(value.trim())) {
